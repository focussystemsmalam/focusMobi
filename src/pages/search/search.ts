...
@Component({
  ...
  template: `
    <ion-list>
      <ion-list-header>
        <ion-icon name="close" (click)="closePopover()"
                  style="font-size: 30px; float: right; margin-right: 5%;"></ion-icon>
      </ion-list-header>

      <ion-grid [formGroup]="searchForm">
        <ion-row class="search-top-buttons">
          <!--<div class="search-top-buttons">-->
          <button class="button" [ngClass]="{active:curDayState == daysStates.D7}" (click)="setDayState(daysStates.D7)">
            Last
            Week
          </button>
          <button class="button" [ngClass]="{active:curDayState == daysStates.MONTH}"
                  (click)="setDayState(daysStates.MONTH)">
            Last Month
          </button>
        ...
        <ion-row class="searchRow">
          <ion-col width-100>
            ...
            <ion-select placeholder="Port of Discharge" formControlName="port" *ngIf="ports && ports.length > 0">
              <ion-option *ngFor="let port of ports.results;let i=index" [value]="port.portCode"><span
                *ngIf="i!=0">({{port.portCode}}) </span>{{port.portName}}
              </ion-option>
            </ion-select>
            <!--</ion-item>-->
          </ion-col>
